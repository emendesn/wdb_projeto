	...
	While TMP1->(!Eof())
		If !Empty(TMP1->OK) 
			...
			If TMP1->B2_QATU > 0
				...
				nlTotd := U_FFilDisp(TMP1->B1_COD,M->C5_DATAINI,M->C5_DATAFIM)
				TMPP->(dbCloseArea())
				
				nlDisp	:= TMP1->B2_QATU - nlTotd
...
## Função:     ## FFilDisp		                             ##
###############################################################
## Descrição:  ## Query para tratar a quantidade disponivel  ##
##			   ## do produto selecionado para a venda        ##
###############################################################
## Parametros  ## clCodProd - Codigo do Produto				 ##
##			   ## dlDtInic  - Data inicial do evento		 ##
##			   ## dlDtFim   - Data final do evento		 	 ##
###############################################################
## Autor :     ## Hermes Ferreira de Almeida                 ##
###############################################################
## Data:       ## 20/01/2010                                 ##
###############################################################
## Palavras Chaves: Disponivel                               ##
#############################################################*/
User Function FFilDisp(clCodProd,dlDtInic,dlDtFim)
	...
	clQryPAZ += "			AND PAZ_PRODUT = '"+Alltrim(clCodProd)+"'"
	clQryPAZ += " 			AND(PAZ_DTINI BETWEEN	'"+ DtoS(dlDtInic)	+"' AND '"	+ DtoS(dlDtFim)+"'"
	clQryPAZ += " 			OR 	PAZ_DTFIM BETWEEN	'"+DtoS(dlDtInic)	+"' AND '"	+ DtoS(dlDtFim)+"'"
	clQryPAZ += " 			OR (PAZ_DTINI <= '"+DtoS(dlDtInic)+"' AND PAZ_DTFIM >= '"+DtoS(dlDtFim) +"' ))"
